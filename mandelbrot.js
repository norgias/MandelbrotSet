...
// Zoom settings
let zoomFactor = 0.89; // Zoom in by 5% each frame
let zoomSpeed = 20; // Fast zoom speed
...
function drawMandelbrot() {
  for (let px = 0; px < width; px++) {
    for (let py = 0; py < height; py++) {
      ...
      // Using shades of green for the color scale
      ctx.fillStyle = `rgb(0, ${color}, 0)`;  // Only green component varies
...
function zoomIn() {
  ...
  drawMandelbrot();
  requestAnimationFrame(zoomIn); // Smooth animation for zooming
}
